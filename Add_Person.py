...
def Draw_Text(img, sTxt, aX=30, aY=30):
    if sTxt == "": 
        return
    cv2.putText(img, str(sTxt), (aX, aY), font, fntSize, (0, 255, 255), fntThickness, cv2.LINE_AA)
...
while True:
    ...
    if DoContrast:
        ...
        image[:,:] = [[min(pixel + dVal, 255) for pixel in row] for row in image[:,:]]
    
    Draw_Text(image, "esc:exit, s:save 50 images")
    ...
    if key == 27:  # esc
        ...
        for i in range(50):
            ...
            incId += 1
            time.sleep(0.1)  # Optional: Add a short delay to avoid capturing identical frames
